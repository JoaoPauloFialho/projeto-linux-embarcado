...
// Variáveis de estado "static" (Visíveis apenas neste arquivo)
...
static uint8_t onewire_ler_byte() {
    uint8_t dado = 0;
    for (int i = 0; i < 8; i++) {
        if (onewire_ler_bit()) {
            dado |= (1 << i); 
...
float ds18b20_executar_fase_B(void) {
    ...
    if (!onewire_reset()) {
        ...
        return -1000.0; // Código de erro genérico
    }
    ...
    return (float)temperatura_raw * 0.0625;
}
